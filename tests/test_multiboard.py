...
def write_board_config(path, data_name):
    ...
    path.write_text(
        yaml.safe_dump({"data_path": f"./{data_name}"}),
        encoding="utf-8",
    )
...
def test_explicit_config_overrides_default_and_keeps_boards_independent(
    ...
    default_config = isolated_app_home / ".kanban-tui.yaml"
    work_config = isolated_app_home / "boards" / "work.yaml"
    write_board_config(default_config, "default.dat")
    write_board_config(work_config, "work.dat")

    default_add = runner.invoke(main, ["add", "default", "task"])
    ...
    default_payload = json.loads(
        runner.invoke(main, ["show", "--format", "json"]).output
    )
    ...
    assert (isolated_app_home / "default.dat").exists()
...
def test_explicit_configure_bootstraps_selected_board(runner, tmp_path):
    ...
    assert raw["data_path"] == str(config_path.with_suffix(".dat").resolve())
...
def test_missing_explicit_config_fails_cleanly(runner, tmp_path):
    ...
    result = runner.invoke(main, ["--config", str(missing), "show"])
...
def test_invalid_explicit_config_fails_cleanly(runner, tmp_path):
    ...
    result = runner.invoke(main, ["--config", str(config_path), "show"])
