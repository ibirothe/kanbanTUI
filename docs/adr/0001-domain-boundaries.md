...
4. `settings.py` composes paths, policy limits and presentation settings into the `AppConfig` used at program entry points. `AppConfig.policy` is the explicit mapping into the domain.
...
- CLI/TUI behavior and the YAML configuration shape remain stable.
...
- `AppConfig` is the composition object at program entry points. Ports can accept its individual path, policy and presentation values without changing the domain.
