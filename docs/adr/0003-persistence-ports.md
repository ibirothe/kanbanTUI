...
`YamlBoardStore` is the production adapter. It maps the existing file lock, codec and atomic writer to the protocols and translates Click-based compatibility errors into application-level `StoreError`. CLI and TUI construct and inject one application object at their entry points. The old config-based functions in `transactions.py` remain temporary compatibility wrappers, but production adapters no longer call them.
